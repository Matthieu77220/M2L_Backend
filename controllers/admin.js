...
// GET - Adherents qu'un admin peut selectionner pour un match
export const getMatchAdherents = (req, res) => {
    const adminSql = `
        SELECT id_club, role
        FROM adherent
        WHERE id_adherent = ?
    ...
    db.query(adminSql, [req.user.id], (adminErr, adminRows) => {
        if (adminErr) {
            console.error('Erreur lors de la recuperation du club admin:', adminErr);
        ...
        if (!isSuperAdmin && admin.id_club == null) {
            return res.status(403).json({ message: "L'admin n'est rattache a aucun club" });
        }

        const adherentsSql = `
            SELECT id_adherent, id_club, prenom, nom
            FROM adherent
            WHERE role = 'utilisateur'
              AND id_club IS NOT NULL
              ${isSuperAdmin ? '' : 'AND id_club = ?'}
            ORDER BY nom, prenom
        `;
        const params = isSuperAdmin ? [] : [admin.id_club];

        db.query(adherentsSql, params, (adherentsErr, adherents) => {
            if (adherentsErr) {
                console.error('Erreur lors de la recuperation des adherents:', adherentsErr);
                return res.status(500).json({ message: 'Erreur serveur' });
            }
...
// POST - Creer un match entre deux adherents autorises
export const createMatch = (req, res) => {
    const idAdherent1 = Number(req.body.id_adherent_1);
    const idAdherent2 = Number(req.body.id_adherent_2);
    const { date_match: dateMatch } = req.body;

    if (!Number.isInteger(idAdherent1) || idAdherent1 <= 0 ||
        !Number.isInteger(idAdherent2) || idAdherent2 <= 0 ||
        !isValidDate(dateMatch)) {
        return res.status(400).json({
            message: 'id_adherent_1, id_adherent_2 et date_match (YYYY-MM-DD) sont obligatoires'
        });
    }

    if (idAdherent1 === idAdherent2) {
        return res.status(400).json({ message: 'Les deux adherents doivent etre differents' });
    }
    ...
    db.query(adminSql, [req.user.id], (adminErr, adminRows) => {
        ...
        const adherentsSql = `
            SELECT id_adherent, id_club, role
            FROM adherent
            WHERE id_adherent IN (?, ?)
        `;

        db.query(adherentsSql, [idAdherent1, idAdherent2], (adherentsErr, adherents) => {
            if (adherentsErr) {
                console.error('Erreur lors de la verification des adherents:', adherentsErr);
                return res.status(500).json({ message: 'Erreur serveur' });
            }

            if (adherents.length !== 2) {
                return res.status(404).json({ message: 'Un des adherents est introuvable' });
            }

            if (adherents.some((adherent) => adherent.role !== 'utilisateur')) {
                return res.status(400).json({ message: 'Un match doit opposer deux utilisateurs' });
            }

            const sameClub = adherents[0].id_club != null &&
                Number(adherents[0].id_club) === Number(adherents[1].id_club);
            const belongsToAdminClub = isSuperAdmin ||
                Number(adherents[0].id_club) === Number(admin.id_club);

            if (!sameClub || !belongsToAdminClub) {
                return res.status(403).json({
                    message: "Vous ne pouvez creer un match qu'entre les adherents autorises d'un meme club"
                });
            }
            ...
            const duplicateSql = `
                ...
                FROM matchs
                WHERE date_match = ?
                  AND ((id_adherent_1 = ? AND id_adherent_2 = ?)
                    OR (id_adherent_1 = ? AND id_adherent_2 = ?))
                LIMIT 1
            `;

            db.query(
                duplicateSql,
                [dateMatch, idAdherent1, idAdherent2, idAdherent2, idAdherent1],
                (duplicateErr, duplicateRows) => {
                    if (duplicateErr) {
                        console.error('Erreur lors de la verification du match:', duplicateErr);
                        return res.status(500).json({ message: 'Erreur serveur' });
                    }

                    if (duplicateRows.length > 0) {
                        return res.status(409).json({ message: 'Ce match existe deja a cette date' });
                    }

                    const insertSql = `
                        INSERT INTO matchs (id_adherent_1, id_adherent_2, date_match)
                        VALUES (?, ?, ?)
                    `;

                    db.query(insertSql, [idAdherent1, idAdherent2, dateMatch], (insertErr, result) => {
                        if (insertErr) {
                            console.error('Erreur lors de la creation du match:', insertErr);
                            return res.status(500).json({ message: 'Erreur serveur' });
                        }

                        return res.status(201).json({
                            message: 'Match cree avec succes',
                            id_match: result.insertId
                        });
                    });
                }
            );
